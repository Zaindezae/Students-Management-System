...
@if ($ex->term == 3)
    <table class="td-left" style="width:100%; border-collapse:collapse; ">
        <tbody>
            <tr>
                <td><strong> {!! strtoupper(Mk::getSuffix(1)) !!} TERM'S AVERAGE: </strong>
                    {{ Mk::getTermAverage($student_id, 1, $year) }}</td>
                <td><strong> {!! strtoupper(Mk::getSuffix(2)) !!} TERM'S AVERAGE: </strong>
                    {{ Mk::getTermAverage($student_id, 2, $year) }}</td>
                <td><strong> {!! strtoupper(Mk::getSuffix(3)) !!} TERM'S AVERAGE: </strong>
                    {{ Mk::getTermAverage($student_id, 3, $year) }}</td>
            </tr>
            <tr>
